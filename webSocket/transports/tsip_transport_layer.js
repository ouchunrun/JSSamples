...
function tsip_transport_layer(o_stack) {
    ...
    this.o_flag = 0;
    this.transport_timer = null;
    this.b_transport_timeout = false;
}
...
tsip_transport_layer.prototype.stop = function () {
    while (this.b_locked){
    	tsk_utils_log_info("tsip_transport_layer.prototype.stop b_locked is deleted");
    }
    ...
    for (var i = 0; i < this.ao_transports.length; ++i) {
        ...
        this.ao_transports[i] = null;
    }
    this.b_transport_timeout = true;
...
tsip_transport_layer.prototype.transport_new = function (e_type, s_host, i_port, s_description, fn_callback) {
    ...
    while (this.b_locked){
    	tsk_utils_log_info("transport_new b_locked is deleted");
    }
    var This = this;
    this.b_locked = true;
    o_transport = new tsip_transport(e_type, This.o_stack, s_host, i_port, s_description, fn_callback, This);
    if (o_transport) {
        this.ao_transports.push(o_transport);
    }
    this.b_locked = false;

    if(This.transport_timer == null)
    {
        tsk_utils_log_info("start transport timer");
        This.fn_callback = fn_callback;
        This.transport_timer = setTimeout( function(){ This.transport_connect_timeout() }, 32000 );
    }
    return o_transport;
}
...
tsip_transport_layer.prototype.transport_remove = function (o_transport) {
    if (o_transport) {
        while (this.b_locked){
        	tsk_utils_log_info("transport_remove b_locked is deleted");
        }
...
tsip_transport_layer.prototype.transport_find = function (o_message) {
	...
	*/
        ...
        /* Find the transport */
        while(this.b_locked){
        	tsk_utils_log_info("transport_find b_locked is deleted");
        }
...
tsip_transport_layer.prototype.handle_re_connection = function(o_data){
	
	tsk_utils_log_info("reconnect wss");
	if(!o_data){
		tsk_utils_log_warn('invalid argument');
		return;
	}
	else if(this.b_transport_timeout){
		tsk_utils_log_warn('transport timeout already');
		return;
	}
	else{
        //tsk_utils_log_info("o_data:\n%j", o_data);
	}

	this.transport_remove(o_data.o_transport);
	this.o_flag++;
	var o_transport = this.transport_new(o_data.e_type, o_data.s_host, o_data.i_port, o_data.s_description, o_data.fn_callback)
	if(o_transport){
		this.o_stack.network.o_transport = o_transport;
		o_transport.start();
	}
}

tsip_transport_layer.prototype.transport_connect_timeout = function () {
    ...
    var This = this;
    tsk_utils_log_error("transport connect timeout");
    this.transport_timer = null;

    if (this.fn_callback) {
    	var length = this.ao_transports.length;
    	for(var i = 0; i < length; i++){
    		if(this.ao_transports[i]){
    			this.ao_transports[i].stop();
    			this.ao_transports[i] = null;
    		}
    	}
    	this.ao_transports.length = 0;
        this.b_transport_timeout = true;
        var fn_callback = this.fn_callback;
        var o_event = new tsip_transport_event(this.o_stack.network.o_transport, tsip_transport_event_type_e.ERROR, "ws_connect_timeout", null);
        setTimeout(function () { fn_callback(o_event) }, 1);

    }
}

tsip_transport_layer.prototype.transaction_connection_successed = function(){
	tsk_utils_log_warn("connect wss succeed");

	if ( this.transport_timer != null ){
        tsk_utils_log_info("clear transport_timer");
